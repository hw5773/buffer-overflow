...
inter = "</table></form><h2>정당한 로그인 방법</h2>정당한 패스워드는 correctpassword입니다. 이를 이용하면 잠금이 풀립니다.<br><h2>서버 소스코드</h2>해커 A모씨 일당은 교수님의 개인 컴퓨터에서 탈취한 보안 서버의 취약점을 발견하였습니다. 아래 코드를 보면 우회할 수 있는 방법이 있다고 하네요.<br>아래 변수 buf에 여러분이 입력한 패스워드가 저장됩니다.<br><br>if (!strncmp((const char *)buf, answer, strlen(answer)))<br>{ <br>&nbsp;&nbsp;&nbsp;&nbsp;printf(\"[*] Password is correct!\n\");<br>&nbsp;&nbsp;&nbsp;&nbsp;pass = 1;<br>} <br>   else <br>{ <br>&nbsp;&nbsp;&nbsp;&nbsp;printf(\"[*] Password is incorrect!\n\");<br>} <br><br>if (pass)<br>{<br>&nbsp;&nbsp;&nbsp;&nbsp;printf(\"[*] The client is logged in and has the root privilege!\n\");<br>}<br><h2>서버에서 사용하는 변수들 주소</h2>"
footer = "</body></html>"
